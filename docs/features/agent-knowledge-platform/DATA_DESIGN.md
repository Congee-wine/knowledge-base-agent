...
- 阶段 3.1 对资料树开放重命名、移动和递归永久删除。移动只更新节点 `parent_id` 与 `updated_at`，服务端递归校验目标不是节点自身或后代。删除先在同一数据库事务中移除 `agent_knowledge_scopes`、分块、处理任务、版本和资料树节点；提交后按已收集的 `storage_key` 删除私有对象。对象删除失败不得回滚数据库删除，应记录可重试的服务端清理失败。
