...
### 阶段 3：前端 SSE 解析与双模式状态机（实现中，自动化验证已通过）
...
已完成：SSE 解析改为兼容 CRLF、多行 `data:` 与末尾无空行的增量解析；活动流保存 `AbortController`、模式和正式助手消息 ID；停止正式生成会先取消浏览器流，再调用中断接口持久化部分回答，预览只取消本地请求。切换智能体、会话或离开预览组件时会取消活动流。尚待阶段 4 的浏览器端页面联调和消息覆盖层验收。
