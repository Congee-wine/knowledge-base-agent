...
> 2026-07-27 状态更正：本计划中 Redis/RQ 聊天事件缓冲、断线恢复订阅和后台聊天生成的实现尝试已撤回，原因是实际 Docker Worker、数据库隧道和真实模型联调未达到可接受的稳定性。当前正式聊天保留已由用户验收的直接 SSE 与停止生成；断线续流延后至知识库完成、部署环境稳定后的独立阶段，不得据本文件将其视为已实现。
...
### 阶段 2B：后端运行生命周期与数据隔离（实现中，服务层验证已通过）
...
已完成：正式聊天会在当前用户消息之前读取最近 10 条内容非空且状态为 `complete` 的历史；`afterSequence > 0` 返回 `STREAM_RESUME_UNAVAILABLE`；新增受认证的助手消息中断接口，持久化客户端上传的已生成片段并标记 `interrupted`。预览仍不调用任何写入方法。尚待阶段 3 将浏览器 `AbortController` 和部分文本接入该中断接口，并完成真实接口与浏览器验收。

### 阶段 3：前端 SSE 解析与双模式状态机（已完成）
...
已完成：SSE 解析改为兼容 CRLF、多行 `data:` 与末尾无空行的增量解析；活动流保存 `AbortController`、模式和正式助手消息 ID；停止正式生成会先取消浏览器流，再调用中断接口持久化部分回答，预览只取消本地请求。切换智能体、会话或离开预览组件时会取消活动流。自动化验证及用户完成的页面层手工测试均已通过；后续阶段 4 只负责完整页面联调与隔离验收。
