...
# 2026-08-14：知识库概览的流式事件

知识库概览不新增 SSE 事件类型，继续复用现有业务事件：

- `status`：依次通知“正在整理知识库概览”“已读取 N 份资料、正在构造回答上下文”“正在生成回答”。
- `sources`：返回概览所依据的已授权资料引用，供前端复用现有来源展示。
- `answer_delta`：模型流式输出文件、主题和可回答问题范围。

因此前端不需要修改 SSE 解析器或事件类型定义；只需按既有状态和来源组件显示即可。
