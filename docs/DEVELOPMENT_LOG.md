...
## 2026-08-05：历史记录打开时为聊天内容预留空间

### 任务目标

避免右侧历史记录抽屉打开时遮挡聊天问答与输入区。

### 实现内容

- 聊天页面在桌面端历史记录打开时增加 260px 右侧预留宽度，与现有抽屉宽度一致。
- 欢迎页、消息列表和输入区在抽屉打开时收窄至最多 900px，并统一向左定位，避免输入区继续贴近或被抽屉遮挡。
- 内容区、输入区和顶部操作在剩余区域内随容器回流，并使用 300ms 过渡匹配抽屉开关动画。
- 窄屏不增加预留，保留覆盖式抽屉以保证主要聊天区域的可用宽度。

### 主要文件

- `frontend/src/pages/ChatPage.tsx`：根据历史抽屉开关状态控制聊天页面的响应式右侧预留。
- `docs/PROJECT_STATUS.md`、`docs/QUESTIONS.md`：同步前端验收状态和用户确认的交互规则。

### 技术方案

不改变固定定位的 Drawer 组件，而是在其打开时由页面容器预留同等宽度。该方案保留抽屉内部滚动、焦点管理和移动端交互，只改变桌面端内容布局。
...
- 需要在包含长 Markdown、表格和较窄桌面窗口的页面中确认回流效果。

## 2026-08-05：更新 AI 管家预设问题

### 任务目标

替换 AI 管家欢迎页中与当前产品能力不匹配的预设提问。

### 实现内容

- 新增内置 AI 管家预设问题数据迁移，题目覆盖资料上传格式、文件夹整理、处理完成时机、资料问答、检索未命中和资料管理。
- 移除在线编辑、文件夹上传、智能客服搭建等当前没有对应功能的引导。
- 为升级和回滚分别保留新旧题目集，确保迁移可逆。

### 主要文件

- `backend/migrations/versions/20260805_0014_refresh_builtin_agent_preset_questions.py`：更新内置 AI 管家的持久化预设问题。
- `docs/PROJECT_STATUS.md`：同步功能状态和数据库迁移记录。
- `docs/QUESTIONS.md`：记录用户确认的产品引导调整结论。

### 技术方案

预设题为数据库种子数据且由 API 返回，采用 Alembic 数据迁移而非前端硬编码，以便已有数据库升级后也立即获得统一内容，同时不影响个人智能体自定义问题。

### 接口或数据变化

- 不新增或修改 API。
- 更新 `agent_preset_questions` 内置 AI 管家记录；需要执行 Alembic 迁移，不涉及表结构变化。

### 验证情况

- 待执行：Python 编译检查、Alembic 历史链路检查及浏览器欢迎页验证。
...
明确 SSE 流式对话在网络断开和页面刷新后的恢复规则，为后续实现提供可验证契约。

### 实现内容

- 确认生成任务不因浏览器断线取消，服务端单次运行上限为 5 分钟。
- 确认页面刷新自动恢复、终态事件缓存 15 分钟，以及有限次数自动重连。
- 明确重新生成不在本次范围内。
- 创建对话续流的需求、前后端、接口、数据和验收设计文档。

### 主要文件

- `docs/features/chat-stream-resumption/`：对话续流的正式设计与验收依据。
- `docs/QUESTIONS.md`：记录并确认用户的续流规则。
- `docs/DECISIONS.md`：记录 Redis Streams 事件回放与 SSE 解耦决策。
- `docs/PROJECT_STATUS.md`、`docs/README.md`：同步项目状态和文档索引。

### 技术方案

规划独立生成任务写入 Redis Streams，SSE 仅回放/订阅事件；PostgreSQL 保存任务与最终消息事实，避免浏览器连接决定模型任务生命周期。

### 接口或数据变化

尚未实施。设计规划新增生成任务持久化实体、Redis Stream 和 `afterSequence` 恢复契约；后续实现时需提供数据库迁移。

### 验证情况

- 已完成需求、接口、数据和验收设计的一致性检查。
- 未执行代码测试：本次仅修改设计文档，未实现功能代码。

### 遗留问题

- 待用户下达实现指令后，按设计开展代码、迁移与端到端验证。
...
- 删除引用摘要的悬停背景变化和资料项的悬停文字变色。

### 主要文件

- `frontend/src/style.css`：聊天资料引用区域的悬停样式。

### 技术方案

保留摘要的折叠点击能力，但不再通过悬停样式强调可交互性；资料项继续作为纯展示文本。
...
取消聊天资料引用项跳转到资料预览页的交互。

### 实现内容

- 将引用资料项由路由链接改为纯文本展示。
- 更新组件测试，确认资料名称保留且不再渲染跳转链接。

### 主要文件

- `frontend/src/features/chat/components/ChatMessageList.tsx`：资料引用项的展示结构。
- `frontend/src/features/chat/components/__tests__/ChatMessageList.test.tsx`：无跳转链接的回归测试。

### 技术方案

保留引用的编号、去重和折叠结构，仅移除路由依赖与可点击语义。
...
- 通过：`pnpm exec tsc --noEmit`。
- 通过：`pnpm test -- --run src/features/chat/components/__tests__/ChatMessageList.test.tsx`（4 项）。
- 通过：`git diff --check`。
...
## 2026-07-29：调整资料引用可读性

### 任务目标

在保留紧凑展示的前提下，提升资料引用摘要和资料名称的可读性。

### 实现内容

- 将引用区域从约三分之二尺寸回调至原始尺寸的约 80%，同步放大摘要、资料文字、编号圆点及布局间距。

### 主要文件

- `frontend/src/style.css`：聊天资料引用区域的尺寸参数。

### 技术方案

仅调整 CSS 视觉比例，不改变引用数据、折叠状态或资料跳转行为。
...
## 2026-07-29：优化聊天资料引用展示

### 任务目标

将资料引用从流式回复过程中的片段列表调整为回复完成后位于答案末尾的简洁资料清单。

### 实现内容

- 仅在助手消息状态为 `complete` 时渲染资料引用，避免回复尚未完成时提前出现。
- 按资料节点去重，保留资料名称与跳转链接，不再展示章节定位和检索片段。
- 新增可折叠的“引用 N 篇资料作为参考”摘要及编号资料列表，资料项可跳转至对应预览页面。
- 增加组件测试，覆盖流式阶段隐藏、完成后显示、资料去重及跳转链接。

### 主要文件

- `frontend/src/features/chat/components/ChatMessageList.tsx`：控制引用显示时机、资料去重和跳转列表渲染。
- `frontend/src/style.css`：引用摘要与编号资料列表的视觉样式。
- `frontend/src/features/chat/components/__tests__/ChatMessageList.test.tsx`：引用展示时机和内容范围的回归测试。

### 技术方案

复用现有消息中的引用数据与资料预览路由，不修改 SSE 或后端响应契约；使用原生 `details` 提供稳定的展开与收起行为，减少额外状态管理。
...
- 待浏览器确认最终视觉间距及引用资料跳转体验。

## 2026-07-29：缩小聊天资料引用区域

### 任务目标

降低答案末尾资料引用摘要和资料列表的视觉体积。

### 实现内容

- 将引用区域的外边距、摘要高度、文字、编号圆点、间距和圆角统一缩小约三分之一。

### 主要文件

- `frontend/src/style.css`：聊天资料引用区域的紧凑样式。

### 技术方案

仅调整现有引用组件的 CSS 尺寸参数，保留资料跳转、去重和折叠交互。
...
## 2026-07-29：修正侧栏智能体选中态

### 任务目标

防止测试个人智能体时，侧栏“我的智能体”中的内置 AI 管家与当前个人智能体同时高亮。

### 实现内容

- 将“我的智能体”列表链接改为精确路由匹配。
- 移除内置 AI 管家的额外路径判断；当前路由仅与对应智能体地址完全一致时才显示选中态。

### 主要文件

- `frontend/src/layouts/AppLayout.tsx`：侧栏个人智能体列表的活动状态判断。
- `docs/PROJECT_STATUS.md`：同步已修正的导航状态。

### 技术方案

保留上方页面导航与下方智能体选择的独立职责：下方列表以具体聊天地址的精确匹配表示当前选中智能体，不以页面路由前缀推断。
...
- 通过：`pnpm exec tsc --noEmit`、`git diff --check`。
- 待执行：浏览器确认 AI 管家/个人智能体之间的高亮切换。

### 遗留问题

- 无新增问题。

## 2026-07-29：移除聊天输入框 Agent 选择按钮

### 任务目标

移除聊天输入框中与当前聊天智能体重复的 Agent 选择入口。

### 实现内容

- 删除输入框底部的“Agent”按钮及其图标依赖。
- 当前聊天智能体继续由侧栏选择的智能体和对应聊天路由唯一决定。

### 主要文件

- `frontend/src/features/chat/components/ChatComposer.tsx`：聊天输入框底部操作区。
- `docs/PROJECT_STATUS.md`：同步当前聊天交互规则。

### 技术方案

不新增本地切换状态或隐藏的二次选择逻辑，避免输入框展示的选择与实际消息发送的智能体上下文不一致。
...
## 2026-07-29：重构聊天执行步骤展示

### 任务目标

将平铺的聊天运行状态改为层级清晰、可展开的执行步骤展示，避免将状态信息误呈现为模型思维链。

### 实现内容

- 连续的“检索资料、构造上下文、未命中或检索失败”事件归纳为“知识库检索”大步骤。
- 大步骤包含两个及以上子事件时，默认展开显示带状态图标的子步骤，并允许用户折叠或重新展开。
- 生成回答等没有子步骤的事件保持为单行状态，不显示展开控件。
- 删除对 Ant Design X `ThoughtChain` 的平铺渲染，使用语义化 `details`/`summary` 保持键盘可访问性。
- 更新消息列表测试以校验生成状态仍显示在对应助手消息中。

### 主要文件

- `frontend/src/features/chat/components/ChatRunSummary.tsx`：执行步骤分组、展开交互与状态图标。
- `frontend/src/style.css`：大步骤、子步骤和展开箭头样式。
- `frontend/src/features/chat/components/__tests__/ChatMessageList.test.tsx`：运行状态渲染回归测试。
- `docs/PROJECT_STATUS.md`、`docs/ARCHITECTURE.md`：同步展示规则和组件职责。

### 技术方案

保留现有 SSE `stage` 事件契约，在前端按阶段归组；不增加模型原始思维链字段，也不改变后端生成或检索流程。
...
- 通过：`pnpm exec tsc --noEmit`、`pnpm test -- --run src/features/chat/components/__tests__/ChatMessageList.test.tsx`（3 项）、`git diff --check`。
- 待执行：浏览器确认展开/折叠行为。

### 遗留问题

- 后续新增联网或文件处理阶段时，需要在前端明确其是否应成为独立大步骤或归入已有组。

## 2026-07-29：修复执行步骤折叠事件崩溃

### 任务目标

修复资料检索阶段出现多个子步骤时，聊天页面因展开状态事件处理异常而空白的问题。

### 实现内容

- 在 `details` 的切换事件回调中立即读取展开状态，再传入状态更新函数。
- 避免异步状态更新阶段访问已失效的 React 合成事件对象。

### 主要文件

- `frontend/src/features/chat/components/ChatRunSummary.tsx`：执行步骤展开状态处理。

### 技术方案

将 DOM 事件值转换为普通布尔值后再进入状态更新，避免状态更新函数依赖事件对象生命周期。
...
## 2026-07-29：优化聊天执行步骤视觉层级

### 任务目标

降低执行步骤展示的卡片噪声，采用更轻量的摘要与展开时间线，提升聊天内容区的可读性。

### 实现内容

- 大步骤改为无背景的单行摘要，完成后显示“已检索知识库 · 命中 N 条资料”。
- 移除父步骤和子步骤的重复大块灰色背景；展开内容改为细分隔线与低对比子步骤。
- 单步骤继续保持无展开控件的紧凑状态。

### 主要文件

- `frontend/src/features/chat/components/ChatRunSummary.tsx`：检索摘要文案与分组展示。
- `frontend/src/style.css`：轻量步骤、展开时间线与图标样式。

### 技术方案

参考成熟对话产品将处理过程放入可展开摘要的惯例，默认强调结果摘要而非过程容器；子步骤仅在用户需要时展开查看。
...
## 2026-07-29：保留完整执行步骤并修复折叠状态

### 任务目标

让同一回复的资料检索和回答生成等大步骤完整保留并同时显示，且子步骤折叠状态可由用户稳定控制。

### 实现内容

- 已完成的本地助手消息保留运行步骤，用于补充服务端历史消息中未持久化的展示信息。
- 消息合并时将本地运行步骤附加到同 ID 的服务端助手消息，避免请求完成后只剩部分步骤。
- 完成的生成步骤改为“已生成回答”，资料检索与生成回答会以并列大步骤显示。
- 自动展开只发生在可折叠分组首次出现时；用户手动收起后不再被后续渲染强制展开。
- 新增回归测试，覆盖完整大步骤显示及收起后重渲染不回弹。

### 主要文件

- `frontend/src/features/chat/components/ChatRunSummary.tsx`：分组首次展开与用户折叠状态。
- `frontend/src/features/chat/components/ChatMessageList.tsx`：已完成步骤标题规范化。
- `frontend/src/features/chat/hooks/useStreamingChat.ts`：结束时固化运行步骤。
- `frontend/src/features/chat/utils/mergeMessages.ts`：将本地步骤补充至服务端消息。
- `frontend/src/features/chat/components/__tests__/ChatRunSummary.test.tsx`：步骤保留与折叠交互测试。
- `docs/PROJECT_STATUS.md`：同步当前客户端展示范围。

### 技术方案

不改变 SSE 或数据库契约。在当前客户端会话内保留已完成助手消息的步骤数据，并以服务端消息作为正文与引用的事实来源；刷新页面后仍按现有接口返回的历史消息展示。
...
- 通过：`pnpm exec tsc --noEmit`、聊天步骤与消息列表测试共 4 项、`git diff --check`。
- 待执行：浏览器确认检索/生成两个大步骤的并列显示及收起后不回弹。

### 遗留问题

- 运行步骤尚未持久化到数据库；刷新页面或重新进入历史会话后，仍只显示服务端现有消息内容与引用。

## 2026-07-29：修复运行步骤导致的流式内容覆盖

### 任务目标

恢复运行步骤显示改动后被服务端空助手消息覆盖的逐段流式回复。

### 实现内容

- 调整消息合并判断顺序：生成中的本地助手消息具有最高优先级，即使其已带有运行步骤也不会被服务端空占位消息替换。
- 仅在流式优先级结束后，才将本地运行步骤补充到同 ID 的服务端助手消息。
- 新增回归测试，覆盖“本地流式文本 + 运行步骤 + 服务端空消息”的合并场景。

### 主要文件

- `frontend/src/features/chat/utils/mergeMessages.ts`：流式与服务端消息的合并优先级。
- `frontend/src/features/chat/utils/__tests__/mergeMessages.test.ts`：带运行步骤的流式内容回归测试。
- `docs/PROJECT_STATUS.md`：同步流式修复状态。

### 技术方案

以流式期间的本地助手消息为内容事实来源；服务端消息仅在本地消息不再处于优先状态时提供持久化正文、引用和历史数据。
...
- 通过：`pnpm exec tsc --noEmit`、聊天消息合并/步骤/列表测试共 14 项、`git diff --check`。
- 待执行：浏览器流式回复复测。
...
## 2026-08-14：知识库概览分支调整

### 任务目标

将只列文件名的资料目录分支调整为可解释的知识库概览。

### 实现内容

- 新增 `knowledge_overview` 策略，识别“知识库有什么”“能回答什么”等概览问题。
- 读取授权范围内的 ready 文档首个已索引片段，并限制为最多 30 份资料、每份 500 字符。
- 模型基于受限上下文输出文件、主题和可回答问题范围；语义检索分支保持原有证据评估。

### 主要文件

- `backend/services/agent_strategy.py`：概览意图路由。
- `backend/services/retrieval.py`：概览资料查询和受限上下文构造。
- `backend/services/agent_runtime.py`：LangGraph 概览路径及流式生成。
- `backend/services/agent_prompt_policies.py`：概览和身份安全提示词策略。

### 接口或数据变化

无。复用既有 SSE 事件、知识库权限范围和文档索引数据。

### 验证情况

- 已执行后端定向单元测试、编译检查和 diff 空白检查。
- 真实文档语料与浏览器端联调待执行。

## 2026-08-14：修复知识库目录与引用不一致

### 任务目标

修复“知识库目录”被误判为普通检索，导致资料清单、模型回答和引用来源不一致的问题。

### 实现内容

- 为“知识库目录”增加概览意图匹配。
- 后端直接流式写入完整文件清单；模型只补充资料概述和可回答问题。
- 文件清单与来源引用使用同一批授权、已索引资料，避免模型凭片段自行补充文件。

### 验证情况

- 已执行 22 项后端路由、检索和 SSE 定向测试，通过。
- 已执行 Python 编译与 diff 空白检查，通过。
- 仍需用截图中的真实资料在浏览器验证索引状态和最终展示。
