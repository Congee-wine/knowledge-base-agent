...
- AI 管家聊天页步骤 2 已完成阶段 2 非流式联调：点击“新会话”仅进入本地空白状态；首次发送才创建会话、写入消息并刷新历史列表；选择历史会话才读取完整消息，发送成功直接更新会话详情缓存。
- 阶段 2 后端基础已完成：智能体 CRUD、默认打开/清空、聊天入口、会话查询和延迟创建消息回显已实现；真实模型 SSE 和前端流式渲染尚未实现。

## 下一步计划

1. 将阶段 2 的延迟创建消息接口升级为真实 SSE：接入模型、流式渲染、取消生成和失败状态。
