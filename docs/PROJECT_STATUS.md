...
功能开发阶段。当前仓库已具备认证基础设施和受认证保护的前端壳页面；知识库、智能体和实际问答能力仍未在代码中实现。

## 当前目标

保持注册、登录、令牌刷新、登出和受保护路由的前后端实现可构建、可运行，并在此基础上明确后续知识库与 AI 功能的实现边界。
...
3. 在独立模块中设计并实现文档导入、解析、切分、向量存储、检索、提示词管理与回答生成；它们目前均为规划中。
4. 将 `AiManagerPage`、智能体和知识库路由替换为实际功能页面，并补充相应接口与测试。
