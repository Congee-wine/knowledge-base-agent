...
## ADR-20260814-08：Agent 知识库问答路由以绑定范围和可用索引分别判定
...
个人 Agent 未绑定资料范围与已绑定范围但没有完成索引的资料，业务含义不同；仅检查 ready 文档会将两种情况错误合并。

### 可选方案

1. 沿用 ready 文档检查，未绑定与无可用资料均返回无资料提示。
2. 个人 Agent 未绑定范围走普通模型回答，已绑定但无 ready 文档返回无资料提示；内置 AI 管家继续以全部资料库开关控制。

### 最终决定

采用方案 2。个人 Agent 未绑定任何资料范围时明确提示未绑定知识库并走普通模型回答；绑定后才启用知识库路径，无 ready 文档时不回退普通模型回答。内置 AI 管家继续通过 `useKnowledgeBase` 使用当前用户全部资料库。

### 选择理由

该规则准确反映 Agent 配置意图，并保留知识库模式的检索必经约束，避免在用户期望基于资料回答时无提示地使用通用模型回答。

### 影响与代价

需要调整 LangGraph 状态和条件边，新增范围绑定查询及回归测试；不涉及数据库迁移、SSE 帧格式或公开 HTTP 接口变化。

### 后续条件

实施后需用未绑定、空范围、文档处理中、全库开关关闭、命中和未命中场景完成端到端验收。
...
# ADR-20260814-02：将资料目录问答升级为受限上下文的知识库概览
...
用户询问“这个知识库有什么”时，只返回文件名不足以说明资料价值；但把全文交给模型既不经济，也会扩大无关内容暴露。

### 可选方案

1. 继续只返回文件名列表。
2. 读取所有全文后由模型汇总。
3. 每份授权 ready 文档提供受限代表性片段，由模型生成概览。

### 最终决定

采用方案 3：最多读取 30 份资料，每份仅使用首个已索引片段的前 500 个字符。

### 选择理由

它能同时提供文件、主题和可回答问题范围，并保持用户/Agent 授权边界；受限片段避免全文上下文带来的延迟和成本。

### 影响与代价

概览不等同于全文摘要，片段不足时模型必须明确说明。具体事实问答仍依赖语义检索和证据评估。
