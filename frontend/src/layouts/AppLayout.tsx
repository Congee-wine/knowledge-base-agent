...
const agents = [
  { name: 'AI管家', to: routes.app.chat, icon: <RobotOutlined />, iconClass: 'bg-gradient-to-br from-cyan-200 to-indigo-500' },
]
...
export function AppLayout({ user, onLogout }: Props) {
  ...
  const handleLogout = async () => { await logout(); onLogout(); navigate(routes.login) }
  return <main className="flex h-screen overflow-hidden bg-slate-100 text-slate-700">
    <aside className={`flex shrink-0 flex-col border-r border-slate-200 bg-white transition-[width] duration-200 ${collapsed ? 'w-[52px]' : 'w-[300px]'}`}>
      ...
      <nav className="mt-3 px-2">{navItems.map(item => <NavLink key={item.to} to={item.to} className={({ isActive }) => { const belongsToChat = item.to === routes.app.chat && location.pathname.startsWith(`${routes.app.chat}/`); return `mb-1 flex h-11 items-center gap-3 rounded-lg px-3 text-sm transition ${isActive || belongsToChat ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-50'}` }}><span className="text-base">{item.icon}</span>{item.label}</NavLink>)}</nav>
