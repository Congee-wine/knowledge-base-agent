...
describe('useStreamingChat', () => {
  ...
  it('appends answer_delta to the real assistant ID', async () => {
    ...
    emit({ type: 'message_start', requestId, sequence: 1, conversationId: 'conv-1', userMessageId: 'u-1', assistantMessageId: 'a-1' })
    emit({ type: 'answer_delta', requestId, sequence: 2, content: 'Hello' })
    emit({ type: 'answer_delta', requestId, sequence: 3, content: ' world' })
  ...
  it('keeps messages and records pending pair on message_end', async () => {
    ...
    emit({ type: 'message_start', requestId, sequence: 1, conversationId: 'conv-1', userMessageId: 'u-1', assistantMessageId: 'a-1' })
    emit({ type: 'answer_delta', requestId, sequence: 2, content: 'answer' })
    emit({ type: 'message_end', requestId, sequence: 3, messageId: 'a-1', generationStatus: 'complete' })
  ...
  it('acknowledgePersistedPair removes only that pair messages', async () => {
    ...
    emit({ type: 'message_start', requestId, sequence: 1, conversationId: 'conv-1', userMessageId: 'u-1', assistantMessageId: 'a-1' })
    emit({ type: 'answer_delta', requestId, sequence: 2, content: 'done' })
    emit({ type: 'message_end', requestId, sequence: 3, messageId: 'a-1', generationStatus: 'complete' })
  ...
  it('acknowledgePersistedPair is a no-op for unknown requestId', async () => {
    ...
    emit({ type: 'message_start', requestId, sequence: 1, conversationId: 'conv-1', userMessageId: 'u-1', assistantMessageId: 'a-1' })
    emit({ type: 'message_end', requestId, sequence: 2, messageId: 'a-1', generationStatus: 'complete' })
  ...
  it('does not mark the previous real assistant as failed when a new request fails before message_start', async () => {
    ...
    emit({ type: 'message_start', requestId: firstRequestId, sequence: 1, conversationId: 'conv-1', userMessageId: 'u-1', assistantMessageId: 'a-1' })
    emit({ type: 'message_end', requestId: firstRequestId, sequence: 2, messageId: 'a-1', generationStatus: 'complete' })
